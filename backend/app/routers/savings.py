...
def _sync_balance(saving: models.Saving, db: Session) -> float:
    ...
    ).order_by(models.Expense.date, models.Expense.id).all()
...
def _recompute_balance_chain(saving: models.Saving, db: Session) -> float:
    """
    Recompute balance_before / balance_after for every transaction in
    chronological order (transaction_date, id) and sync current_balance.
    Returns the minimum running balance seen (used to detect negative-balance violations).
    """
    transactions = db.query(models.SavingTransaction).filter(
        models.SavingTransaction.saving_id == saving.id
    ).order_by(
        models.SavingTransaction.transaction_date,
        models.SavingTransaction.id
    ).all()
    ...
    for tx in transactions:
        ...
        if tx.transaction_type in ("deposit", "interest"):
            running += tx.amount
        else:
            running -= tx.amount
    ...
    saving.current_balance = running
    return min_balance
...
def get_total_balance(
    ...
    if any_synced or db.new or db.dirty:
        db.commit()
...
def get_aggregated_monthly_summary(
    ...
    for i in range(months - 1, -1, -1):  # oldest to newest
        ...
        for saving in savings_accounts:
            ...
            closing = last_tx.balance_after if last_tx else 0.0
...
def update_saving(
    ...
    if "interest_rate" in update_data:
        applied = db.query(models.SavingTransaction).filter(
        ...
        if applied:
            raise HTTPException(
                ...
                detail="Cannot change interest_rate after interest has already been applied"
            )
...
def get_saving_monthly_summary(
    ...
    for i in range(months - 1, -1, -1):  # oldest to newest
        ...
        closing_balance = last_tx.balance_after if last_tx else 0.0
...
def create_saving_transaction(
    ...
    # Find the balance at the point of the transaction date (not just current_balance)
    prev_tx = db.query(models.SavingTransaction).filter(
        models.SavingTransaction.saving_id == saving_id,
        models.SavingTransaction.transaction_date <= transaction.transaction_date
    ).order_by(
        models.SavingTransaction.transaction_date.desc(),
        models.SavingTransaction.id.desc()
    ).first()
    balance_at_date = prev_tx.balance_after if prev_tx else 0.0

    if transaction.transaction_type == "withdrawal" and transaction.amount > balance_at_date:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Insufficient balance at {transaction.transaction_date}. Available: {balance_at_date}"
        )

    # Placeholder values — _recompute_balance_chain will correct them
    ...
    _recompute_balance_chain(saving, db)
...
def withdraw_from_saving(
    ...
    # Check balance at the point of the withdrawal date
    prev_tx = db.query(models.SavingTransaction).filter(
        models.SavingTransaction.saving_id == saving_id,
        models.SavingTransaction.transaction_date <= trans_date
    ).order_by(
        models.SavingTransaction.transaction_date.desc(),
        models.SavingTransaction.id.desc()
    ).first()
    ...
    if body.amount > balance_at_date:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Insufficient balance at {trans_date}. Available: {balance_at_date}"
        )
...
def delete_saving_transaction(
    ...
    # Simulate the chain without this transaction to check for negative balances
    remaining = db.query(models.SavingTransaction).filter(
        models.SavingTransaction.saving_id == saving_id,
        models.SavingTransaction.id != transaction_id
    ).order_by(
        models.SavingTransaction.transaction_date,
    ...
    for tx in remaining:
        ...
        if running < 0:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Cannot delete: would result in negative balance in the transaction chain"
            )
