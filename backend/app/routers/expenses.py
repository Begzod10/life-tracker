...
def restore_expense(
    ...
    # Recreate savings withdrawal if this was a savings-funded expense
    if db_expense.source == "savings" and db_expense.saving_id and not db_expense.saving_transaction_id:
        saving = db.query(models.Saving).filter(
            models.Saving.id == db_expense.saving_id,
            models.Saving.person_id == current_user.id,
            models.Saving.deleted == False
        ).first()
        if saving:
            if saving.current_balance < db_expense.amount:
                db.rollback()
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Insufficient savings balance to restore expense. Available: {saving.current_balance}"
                )
            balance_before = saving.current_balance
            ...
            saving_tx = models.SavingTransaction(
                ...
                balance_before=balance_before,
                balance_after=balance_after,
                description=f"Expense: {db_expense.name} (expense_id={db_expense.id})"
            )
            db.add(saving_tx)
            db.flush()
            db_expense.saving_transaction_id = saving_tx.id
...
def update_expense(
    ...
    # Sync linked SavingTransaction if amount or date changed
    if db_expense.source == "savings" and db_expense.saving_transaction_id:
        ...
        if 'amount' in update_data or 'date' in update_data:
            ...
            if saving_tx:
                saving_tx.amount = new_amount
                saving_tx.transaction_date = new_date
                db.flush()
                saving = db.query(models.Saving).filter(
