...
import { useTaskCreate, useTasksList } from '@/lib/hooks/use-tasks'
...
function PlatformPageContent() {
    ...
    const handleSelectCategory = (id: string | null) => {
        if (id === 'finances' && user?.id) {
            router.push(`/platform/${user.id}/finances`)
            return
        }

        if (id === 'health' && user?.id) {
            router.push(`/platform/${user.id}/health`)
            return
        }
            ...
            <div className="container mx-auto px-6 py-6">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        ...
                        {/* Stats Cards */}
                        <StatsSection category={category} timeRange={timeRange} overviewStats={overviewStats} />
                    ...
                    <div className="space-y-6">
                        ...
                        <ProgressSection category={category} overviewStats={overviewStats} />
                    </div>
                </div>
            </div>
        </motion.div>
...
// Stats Section
function StatsSection({ category, timeRange, overviewStats }: {
    category: typeof categories[0]
    timeRange: string
        ...
        return (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <Card className="bg-[#1a1b26] border-[#2a2b36] p-6">
                    <div className="text-sm text-gray-400 mb-2">Added This {timeRange}</div>
                    <div className="text-3xl font-bold text-white mb-1">12</div>
                    <div className="text-sm text-green-400">+25% from last {timeRange}</div>
                </Card>

                <Card className="bg-[#1a1b26] border-[#2a2b36] p-6">
                    <div className="text-sm text-gray-400 mb-2">Completed</div>
                    <div className="text-3xl font-bold text-white mb-1">8</div>
                    <div className="text-sm text-green-400">+15% completion rate</div>
                </Card>

                <Card className="bg-[#1a1b26] border-[#2a2b36] p-6">
                    <div className="text-sm text-gray-400 mb-2">In Progress</div>
                    <div className="text-3xl font-bold text-white mb-1">4</div>
                    <div className="text-sm text-gray-400">67% of total</div>
                </Card>
            </div>
        )
    }
...
function ItemsSection({ category, viewMode, setViewMode, userId }: {
    ...
    // Deleted goals query
    console.log('ItemsSection userId:', userId);
    const { data: deletedGoals, isLoading: isLoadingDeleted } = useDeletedGoalsList(userId as string)
    console.log('Deleted goals hook result:', { deletedGoals, isLoadingDeleted });
...
// Progress Section
function ProgressSection({ category, overviewStats }: {
    category: typeof categories[0]
    overviewStats?: GoalOverviewStats
}) {
    const completionPercentage = category.id === 'goals'
        ? Math.round(overviewStats?.average_completion || 0)
        : 67 // Default value for other categories for now
            ...
            <div className="space-y-4">
                ...
                <div className="text-center">
                    <div className="text-sm text-green-400 font-semibold">
                        {category.id === 'goals' ? 'Average across all goals' : '↑ +12% this week'}
                    </div>
