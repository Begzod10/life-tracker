...
const JobCard: React.FC<{ job: Job; onDelete: (id: string | number) => void }> = ({ job, onDelete }) => (
    <motion.div
...
const DeleteJobButton: React.FC<{ job: Job; onDelete: (id: string | number) => void }> = ({ job, onDelete }) => {
...
const EditJobButton: React.FC<{ job: Job; personId: string | number }> = ({ job, personId }) => {
    const [open, setOpen] = useState(false)
            ...
            {displaySalaries.length === 0 ? (
                ...
                <div className="grid gap-4">
                    {displaySalaries.map((salary: SalaryMonth) => (
                        <Card key={salary.id} className="bg-[#1a1b26] border border-[#2a2b36] p-6 rounded-xl hover:border-[#3a3b46] transition-all">
                            <div className="flex justify-between items-start mb-4">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2">
                                        <h3 className="text-lg font-semibold text-white">{getJobName(salary)}</h3>
                                        {!showDeleted && <DeleteSalaryButton salary={salary} />}
                                    </div>
                                    <p className="text-sm text-gray-400">Month: {salary.month}</p>
                                </div>
                                <div className="text-right">
                                    <div className="text-lg font-bold text-teal-400">
                                        {salary.net_amount.toLocaleString()} SUM
                                    </div>
                                    <p className="text-xs text-gray-500">Net Received</p>
                                </div>
                            </div>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4 pt-4 border-t border-[#2a2b36]">
                                <div>
                                    <p className="text-xs text-gray-500">Gross Salary</p>
                                    <p className="text-sm text-gray-300">{salary.salary_amount.toLocaleString()} SUM</p>
                                </div>
                                <div>
                                    <p className="text-xs text-gray-500">Deductions</p>
                                    <p className="text-sm text-red-400">{salary.deductions.toLocaleString()} SUM</p>
                                </div>
                                <div>
...
const ExpenseFormModal: React.FC<{ personId: string | number }> = ({ personId }) => {
    ...
    const salaryOptions = salaries.map((s: SalaryMonth) => {
        ...
        if (job) {
            return {
                value: s.id.toString(),
                label: `${job.name} ${job.company} - ${s.net_amount.toLocaleString()} ${job.currency || 'UZS'}`
            }
        }
        return {
            value: s.id.toString(),
            label: `Unknown Job - ${s.net_amount.toLocaleString()} UZS`
        }
    })
    ...
    const [formData, setFormData] = useState({
        ...
        description: '',
        salary_month_id: salaries.length > 0 ? salaries[0].id : 1
    })

    // Update default salary_month_id when salaries are loaded
    React.useEffect(() => {
        if (salaries.length > 0 && formData.salary_month_id === 1) {
            setFormData(prev => ({ ...prev, salary_month_id: salaries[0].id }))
        }
    }, [salaries, formData.salary_month_id])
        ...
        <Dialog>
            ...
            <DialogContent className="bg-[#1a1b26] border border-[#2a2b36]">
                ...
                <div className="flex justify-end gap-3 mt-6">
                    <DialogTrigger asChild>
                        <Button variant="outline" className="border-[#2a2b36] text-white hover:bg-[#2a2b36]">
                            Cancel
                        </Button>
                    </DialogTrigger>
                    <DialogTrigger asChild>
...
export default function FinancesPage() {
    ...
    // Use API data if available, otherwise fall back to 0 or mock for initial state (though we load first)
    const totalBalance = summary ? summary.total_savings : 0
            ...
            <div className="max-w-7xl mx-auto">
                ...
                >
                    <OverviewCard
                        ...
                        trend={summary?.total_savings_trend ?? 0}
                        delay={0}
                    />
                    <OverviewCard
                        label="Monthly Income"
                        value={`${monthlyIncome.toLocaleString()} UZS`}
                        trend={summary?.total_income_trend ?? 0}
                        delay={1}
                    />
                    <OverviewCard
                        label="Monthly Expenses"
                        value={`${monthlyExpenses.toLocaleString()} UZS`}
                        trend={summary?.total_expenses_trend ?? 0}
                        delay={2}
                    />
                    <OverviewCard
                        label="Savings Rate"
                        value={`${savingsRate}%`}
                        trend={summary?.savings_rate_trend ?? 0}
                        delay={3}
                    />
                </motion.div>
